...
To perform a "NOT LIKE" query, you can use the `Ne` operator in combination with a `Like` or `Ilike` operator. Here is an example:
...
This query will return all `httpEvents` where the `requestUri` does not contain the "unwantedPattern" string.
