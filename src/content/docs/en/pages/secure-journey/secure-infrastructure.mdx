...
Origin Shield works as a list of IP/CIDR addresses used in Azion's Edge, thus providing assurance of safe IPs to allow access. The list is maintained by Azion and available via Azion Console and API, but each client must create automation to ensure their security policies are always up-to-date with the Origin Shield addresses.

It is important to note that IP addresses in the Origin Shield can change frequently. When new IP addresses are added, they are quarantined, meaning they are isolated for a period to ensure stability and security before being fully integrated. This process requires clients to adjust their firewall configurations accordingly to accommodate the new addresses.
...
[TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO MANAGE FIREWALL CONFIGURATIONS FOR NEW IP ADDITIONS]