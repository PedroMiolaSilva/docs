...
- For the `Countries` type, a selection list will be presented. The `NetworkList America` includes countries in North, Central, and South America. Users can verify or customize these lists by accessing the selection list.
...
For `ASN` and `IP/CIDR` list types, a typing field will be displayed. List items must be separated by line and you must write one address per line. Duplicated items will be deleted. For the `Countries` type, a selection list will be presented. The `NetworkList America` includes countries in North, Central, and South America. Users can verify or customize these lists by accessing the selection list.
...
- To configure a rule for a specific user-agent, use the `is not equal` operator for exact matches in the configuration.
