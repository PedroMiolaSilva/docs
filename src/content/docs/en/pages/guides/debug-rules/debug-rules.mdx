...
**Regex Functionality:**
At this time, the ability to query requests using a regex configured in the rate limit is not available. Instead, you can use the `LIKE` operator for filtering.

**Using the LIKE Operator in GraphQL:**
To filter using the `LIKE` operator, modify the GraphQL query as follows:
...
This query filters requests where the `requestUri` contains the path `/example/path`. The `%` symbol is used as a wildcard character.
