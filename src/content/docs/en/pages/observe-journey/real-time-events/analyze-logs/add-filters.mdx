...
To perform queries using more complex patterns, you can utilize regular expressions in combination with the `LIKE` parameter. [TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO USE REGEX WITH LIKE PARAMETER]

You can also search for more complex queries with the `AND`, `OR`, and `NOT` notations to combine the fields.

As you're querying for a variable and a value, it's important to understand the available variables and the [type of values it can accept](/en/documentation/products/observe/real-time-events/#data-sources).
