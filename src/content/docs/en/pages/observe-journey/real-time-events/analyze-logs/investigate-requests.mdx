...
## Step 3. Using Regex in Queries

To query requests using a regex configuration, you can utilize the `requestUri` field in your GraphQL queries. This allows you to filter requests based on patterns in the URI.

1. On [Real-Time Events GraphQL's playground](https://manager.azion.com/events/graphql), you can use a query like the following:

```graphql
query requestsInvestigationWithRegex {
  httpEvents(
    limit: 100,
    filter: {
      tsRange: {begin:"2024-03-20T10:10:10", end:"2024-03-27T10:10:10"}

      requestUriMatches: "/api/.*"

    }
    aggregate: {count: requestUri}
    groupBy: [requestUri, status]
    orderBy: [count_DESC]
  )
  {
    requestUri
...
2. This query will return requests where the `requestUri` matches the pattern `/api/.*`, allowing you to filter based on URI patterns.
