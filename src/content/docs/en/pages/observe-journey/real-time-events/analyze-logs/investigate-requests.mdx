...
## Filtering Logs Using LIKE Parameter

To perform queries using patterns like regex, you can use the `LIKE` parameter in your GraphQL queries to filter logs in Real-Time Events. For instance, if you want to filter requests where the `requestUri` matches a specific pattern, you can use:

```graphql
query requestsLikeFilter {
  httpEvents(
    limit: 100,
    filter: {
      requestUri_like: "^/api/.*"
...
This query filters all requests with `requestUri` starting with `/api/`.
...
## Debug Rules Feature for Improved Logging

The Debug Rules feature can be enabled within the Edge Application and Edge Firewall to improve logging and facilitate investigation of issues. 

1. Access your Edge Application or Edge Firewall.
2. Navigate to the Debug Rules section.
3. Enable the Debug Rules feature.
4. [TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO USE DEBUG RULES FOR LOGGING].
