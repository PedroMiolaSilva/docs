...
4. To query requests using a regex, modify your query filter to include a regex condition. For example, to filter requests with a specific pattern in the `requestUri`, you can use:

```graphql
query requestsInvestigationRegex {
  httpEvents(
    limit: 100,
    filter: {
      ...
      requestUriRegex: "^/api/v[0-9]+/"
    }
    aggregate: {count: requestUri}
    groupBy: [host, requestUri, status]
    orderBy: [count_DESC]
  )
  {
    requestUri
