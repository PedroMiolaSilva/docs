...
:::note
**Current Limitation**: Regex functionality for log searches is currently limited. As an alternative, consider using the `LIKE` parameter for querying requests.
:::
...
:::note
To effectively filter rate-limit rules, ensure that Debug Rules are enabled in the Edge Application or Firewall. This will populate the traceback field necessary for filtering.
:::
