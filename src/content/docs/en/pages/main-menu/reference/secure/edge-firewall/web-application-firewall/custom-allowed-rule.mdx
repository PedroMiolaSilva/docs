...
To perform log queries using regex configured in the rate limit for file extensions like .js, .css, and .png, there are limitations to using regex directly. Instead, it is recommended to use the `LIKE` parameter for more effective log queries.

### Using the LIKE Parameter

To query for specific file extensions such as `.js`, `.css`, and `.png`, you can structure your query using the `LIKE` parameter. Here’s an example:

```graphql
{
  logs(filter: {field: "extension", value: "%.js", operator: "LIKE"}) {
    field1
    field2
...
This query will allow you to search for logs related to `.js` files. You can adjust the `value` parameter for `.css` or `.png` files accordingly.

### Enabling Debug Rules

To improve rule visibility in logs for Edge Applications and Edge Firewall, enabling Debug Rules is essential. Follow these steps:

1. Navigate to your Edge Application settings.
2. Locate the Debug Rules option.
3. Enable Debug Rules to enhance logging capabilities and provide better insights into rule processing.

[TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO ENABLE DEBUG RULES IN EDGE APPLICATIONS AND EDGE FIREWALL]
