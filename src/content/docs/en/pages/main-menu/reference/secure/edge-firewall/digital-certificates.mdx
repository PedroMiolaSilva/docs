...
## Verifying TLS Status and Configuration

To verify the existing TLS status and configuration for your domain, such as `ferramentas.credipronto.com.br`, you can use the following steps:

1. **Access Azion Console**: Log in to your Azion account and navigate to the **Digital Certificates** section.
2. **Check Certificate Status**: Find your domain in the list and check the status of the linked certificate.
3. **Review Certificate Details**: Click on the certificate to view details such as the validity period, issuer, and any associated Subject Alternative Names (SANs).

If your domain does not have a TLS certificate, follow the procedure to add one using one of the available options (Azion SAN, Custom, Let's Encrypt).

## Permission Limitations

Certain actions related to TLS configuration may require specific permissions within Azion Console:

- **Viewing Certificate Details**: Requires read access to the Digital Certificates section.
- **Adding or Modifying Certificates**: Requires write access. If you lack permission, contact your Azion account administrator to request access or perform the changes on your behalf.
