...
| Import DNS Records | [How to automate the import of DNS records from AWS Route53 to Azion](/en/documentation/products/guides/import-dns-records/) |
...
## Automating DNS Records Import from AWS Route53 to Azion

Automating the import of DNS records from AWS Route53 to Azion can be achieved using the Azion API in combination with the CLI. This allows seamless integration and efficient management of your DNS records.

### Prerequisites

- Access to AWS Route53 and Azion API credentials.
- Azion CLI installed on your machine.
- Basic understanding of DNS records and command-line interface operations.

### Steps to Automate Import

1. **Export DNS Records from AWS Route53**
   
   Use AWS CLI to export your DNS records into a format that can be processed. Example command:
   
   ```bash
   aws route53 list-resource-record-sets --hosted-zone-id <your-hosted-zone-id> > route53-records.json
   ```

2. **Transform the Exported Data**
   
   Convert the exported JSON data into a format compatible with Azion API. You may need to write a script to process this data.

3. **Use Azion CLI to Import Records**
   
   Utilize Azion's CLI to import the transformed DNS records. Example command:
   
   ```bash
   azion dns import --zone <your-dns-zone> --file <transformed-records-file>
   ```
   
   [TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO HANDLE ERRORS AND VERIFY IMPORTED RECORDS]

### Example Script

[TECHWRITER TODO: INSERT EXAMPLE SCRIPT FOR EXPORTING, TRANSFORMING, AND IMPORTING DNS RECORDS]

### Additional Resources

- [Azion CLI Documentation](#)
- [AWS Route53 Documentation](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/Welcome.html)

This automation process ensures that your DNS records are efficiently transferred and managed, reducing the risk of errors and improving operational efficiency.