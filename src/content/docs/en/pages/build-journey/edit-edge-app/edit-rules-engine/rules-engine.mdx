...
When defining criteria for rules, it's important to select variables and operators that precisely match the conditions you want to apply. For example, using the `${uri}` variable with the **starts with** operator and the `/` argument specifies a rule targeting all requests with URIs starting with `/`. Be aware that "NetworkList America" is not a strict criterion and should be used according to your application requirements.
